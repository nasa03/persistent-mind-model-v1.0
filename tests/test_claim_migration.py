...
def test_migrate_claims_from_history():
    ...
    log.append(
        kind="assistant_message", content="VALUE: Stability is paramount", meta={}
    )
...
def test_migrate_claims_preserves_claim_ids():
    ...
    claim1 = json.loads(
        [e for e in events1 if e["kind"] == "claim_register"][0]["content"]
    )
    claim2 = json.loads(
        [e for e in events2 if e["kind"] == "claim_register"][0]["content"]
    )
